...
<script type="text/javascript">
	...
	function FriendService($http, $q) {
		...
		function acceptFriendRequset(friendId) {
			...
			return $http.get(BASE_URL + 'acceptfriendrequest/' + friendId)
		...
		function rejectFriendRequset(friendId) {
			...
			return $http.get(BASE_URL + 'rejectfriendrequest/' + friendId)
					.then(function(response) {
						return response.data;
					}, function(errResponse) {
						return $q.reject(errResponse);
		...
		function removeFriend(friendId){
			console.log("inside the remove friend request service");
			return $http.post(BASE_URL + 'removefriend', friendId)
					.then(function(response) {
						return response.data;
					}, function(errResponse) {
						return $q.reject(errResponse);
	...
	function myCtrl(FriendService, $scope) {
		...
		function viewFriendRequset() {
			FriendService.viewFriendRequset().then(function(response) {
		...
		function acceptFriendRequset(friendId) {
			FriendService.acceptFriendRequset(friendId).then(
					function(response) {
						$scope.friendrequest = response;
						getFriends();
						countFriendRequests();
		...
		function rejectFriendRequset(friendId) {
			FriendService.rejectFriendRequset(friendId).then(
					function(response) {
						$scope.friendrequest = response;
					}, function(errResponse) {
						console.log('Error fetching Users');
		...
		function countFriendRequests() {
			FriendService.countFriendRequests().then(function(response) {
		...
		function removeFriend(friendId){
			...
			FriendService.removeFriend(friendId).then(
					function(response) {
	...
	<div class="container">
		<h2>
			<img
				...
				alt="logo" width="30" height="30">&nbsp Friends
		</h2>
		<div class="col-md-4 col-md-offset-4">


			<button class="btn btn-success " ng-click="viewFriendRequset();">
			...
			<div>
				...
				<div class="panel-group" ng-show="friends">
					...
					${pageContext.request.contextPath}/resources/images/{{user.friendId.email}}.jpg "
								...
								onerror="this.src='${pageContext.request.contextPath}/resources/images/user.jpg'"
								width="120" height="120" id="sm_profilepic" /> <span
								style="font-size: x-large">{{user.friendId.username}}</span><br>
							<i class="fa fa-envelope-o" aria-hidden="true"></i>&nbsp;&nbsp;{{user.friendId.email}}<br />

							<i class="fa fa-map-marker" aria-hidden="true"></i>&nbsp;&nbsp;{{user.friendId.city}}<br />

							<i class="fa fa-phone" aria-hidden="true"></i>&nbsp;&nbsp;{{user.friendId.phone}}<br />
						...
						<div class="panel-footer">
							<a
								href="http://localhost:9999/Talk/talk/friendchat/{{user.friendId.userId}}"
								class="btn btn-success btn-sm" ng-disabled="!user.online"
								role="button"><i class="fa fa-paper-plane-o"
								aria-hidden="true"></i>&nbsp SEND MESSAGE</a>
								
								<button class="btn btn-danger btn-sm pull-right" ng-click="removeFriend(user.friendId.userId)">
